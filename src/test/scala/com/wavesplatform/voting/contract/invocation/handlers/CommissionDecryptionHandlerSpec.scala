...
import com.wavesplatform.voting.contract.{PollType, ServerTrait, Status, VotingBase, VotingError, VotingStatus}
...
class CommissionDecryptionHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  "all validations passed" in {
    ...
    val callRes = CommissionDecryptionHandler.call(contractTransaction, contractState)
    callRes shouldBe 'right
    callRes.right.get should contain theSameElementsAs Seq(
      DataEntry("COMMISSION_DECRYPTION", DataEntry.Value.StringValue(contractTransaction.id)))
  }
