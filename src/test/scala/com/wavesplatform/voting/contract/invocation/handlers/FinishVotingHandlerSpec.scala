...
import com.wavesplatform.voting.contract.{PollType, ServerTrait, Status, VotingBase, VotingStatus, _}
import org.scalatest.freespec.AnyFreeSpec
import org.scalatest.matchers.should.Matchers

class FinishVotingHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  "all validations passed" in {
    ...
    val callRes = FinishVotingHandler.call(contractTransaction, contractState)
    callRes shouldBe 'right

    val updatedVotingBase = parseJson[VotingBase](callRes.right.get.head.value.stringValue.get).right.get

    updatedVotingBase.status shouldBe VotingStatus.Completed
    updatedVotingBase.dateEnd shouldNot be(empty)
  }

  "check status halted" in {
    val contractState: ContractKeysResponse =
      ContractKeysResponse(
        servers.map(_.toDataEntry) ++ Seq(
          votingBase.copy(dateStart = Instant.now().plus(2, ChronoUnit.DAYS).truncateToSeconds()).toDataEntry))
    val callRes = FinishVotingHandler.call(contractTransaction, contractState)
    callRes shouldBe 'right

    val updatedVotingBase = parseJson[VotingBase](callRes.right.get.head.value.stringValue.get).right.get

    updatedVotingBase.status shouldBe VotingStatus.Halted
    updatedVotingBase.dateEnd shouldNot be(empty)
  }
