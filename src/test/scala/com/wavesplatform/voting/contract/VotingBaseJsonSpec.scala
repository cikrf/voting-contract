package com.wavesplatform.voting.contract

import java.math.BigInteger
import java.time.Instant

import com.wavesplatform.voting.contract.util.TestDateTime._
import org.scalatest.freespec.AnyFreeSpec
import org.scalatest.matchers.should.Matchers
import play.api.libs.json._

class VotingBaseJsonSpec extends AnyFreeSpec with Matchers {

  private val votingBase: VotingBase = VotingBase(
    pollId = "pollIdValue",
    pollType = PollType.Blind,
    bulletinHash = "BulletinHash",
    dimension = Vector(2, 2, 2),
    status = VotingStatus.Completed,
    dateStart = Instant.now(),
    dateEnd = Some(Instant.now()),
    k = 10,
    dkgRound = 5,
    blindSigModulo = new BigInteger(
      "",
      16),
    blindSigExponent = new BigInteger("10001", 16)
  )

  "json read/write" in {
    val votingJsValue = Json.toJson(votingBase)

    val fromJson = Json.fromJson[VotingBase](votingJsValue)
    fromJson.isSuccess shouldBe true
    val parsedVoting = fromJson.get
    parsedVoting shouldEqual votingBase.copy(
      dateStart = votingBase.dateStart.truncateToSeconds(),
      dateEnd = votingBase.dateEnd.map(_.truncateToSeconds()))
  }

  "json read/write (check nullable field)" in {
    val votingBaseWithNone = votingBase.copy(dateEnd = None)
    val votingJsValue      = Json.toJson(votingBaseWithNone)

    val fromJson = Json.fromJson[VotingBase](votingJsValue)
    fromJson.isSuccess shouldBe true
    val parsedVotingBase = fromJson.get

    parsedVotingBase shouldEqual votingBaseWithNone.copy(dateStart = votingBaseWithNone.dateStart.truncateToSeconds())
  }

}
