...
/**
  * Проверки:
  *
  * Проверить что статус отправителя в ключе SERVER_<publicKey>, его тип SERVER_<publicKey>.type равен decrypt или main и SERVER_<publicKey>.status равен active
...
object DecryptionHandler extends CallHandler {
  ...
  def call(
    ...
    for {
      ...
      _          <- VotingValidators.serverCanVote(contractTransaction, servers)
      ...
      DataEntry(
        s"DECRYPTION_${contractTransaction.senderPublicKey}",
