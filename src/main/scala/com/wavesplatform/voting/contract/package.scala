...
package object contract {
  ...
  implicit val mapReads: Reads[Map[Int, Point]] = new Reads[Map[Int, Point]] {
    def reads(jv: JsValue): JsResult[Map[Int, Point]] =
      JsSuccess(jv.as[Map[String, JsObject]].map {
        case (k, v) =>
          Integer.parseInt(k) -> v.as[Point]
      })
  }

  implicit val mapWrites: Writes[Map[Int, Point]] = new Writes[Map[Int, Point]] {
    def writes(map: Map[Int, Point]): JsValue =
      Json.obj(map.map {
        case (s, o) =>
  ...
  implicit val mapFormat: Format[Map[Int, Point]] = Format(mapReads, mapWrites)
